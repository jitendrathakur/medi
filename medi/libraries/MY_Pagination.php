...
class MY_Pagination extends CI_Pagination
{
	...
	var $full_tag_open		= '';
	var $full_tag_close		= '';
	var $start_tag_open =  '<div class="toolkit_bot_pagi">';
	var $start_tag_close = '</div>';
	var $first_tag_open		= '';
	var $first_tag_close	= '';
	var $last_tag_open		= '<a>';
    ...
    {  
      ...
      if ($this->show_count)
      {
         ...
         $info = '<div class="toolkit_sho_res"> ' . ( $curr_offset + 1 ) . ' - ' ;
     
//propeblem in listing caption	    //if( ( $curr_offset + $this->per_page ) < ( $this->total_rows -1 ) )
         if( ( $curr_offset + $this->per_page ) < ( $this->total_rows ) )
            $info .= $curr_offset + $this->per_page;
         else
            $info .= $this->total_rows;

         $info .= ' of ' . $this->total_rows . ' Listings </div> ';

         $output .= $info . $this->start_tag_open;
      }
           ...
           // $output .= $this->prev_tag_open.'<a '.$paging_function.'href="'.$this->base_url.$i.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
		    ...
		    if($this->action_id!=""){
				if($i=="")
					...
					$pg_prev = $i;
				$output .= $this->prev_tag_open.'<a onclick="'.$this->paging_function.'('.$pg_prev.','.$this->action_id.');" href="Javascript:void(0);" class="prev">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
			else{
		     	$output .= $this->prev_tag_open.'<a onclick="'.$this->paging_function.'('.$i.');" href="Javascript:void(0);" class="prev">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
			...
			if ($i >= 0)
			{
				...
				//	$output .= $this->num_tag_open.'<a '.$paging_function.'href="'.$this->base_url.$n.'">'.$loop.'</a>'.$this->num_tag_close;
					...
					if($n=="")
						...
						$pg = $n;
					$output .= $this->num_tag_open.'<a onclick="'.$this->paging_function.'('.$pg.','.$this->action_id.');" href="Javascript:void(0);">'.$loop.'</a>'.$this->num_tag_close;
					}
					else{
					$output .= $this->num_tag_open.'<a onclick="'.$this->paging_function.'('.$n.');" href="Javascript:void(0);">'.$loop.'</a>'.$this->num_tag_close;
					}
				}
			}
        } 
        ...
        //    $output .= $this->next_tag_open.'<a  '.$paging_function.'href="'.$this->base_url.($this->cur_page * $this->per_page).'">'.$this->next_link.'</a>'.$this->next_tag_close;        
	 	...
	 	if($this->action_id!="")
			$output .= $this->next_tag_open.'<a  onclick="'.$this->paging_function.'('.($this->cur_page * $this->per_page).','.$this->action_id.');" href="Javascript:void(0);" class="nxt">'.$this->next_link.'</a>'.$this->next_tag_close;	
		else
    		$output .= $this->next_tag_open.'<a  onclick="'.$this->paging_function.'('.($this->cur_page * $this->per_page).');" href="Javascript:void(0);" class="nxt">'.$this->next_link.'</a>'.$this->next_tag_close;
	    }
		...
		$output .= $this->start_tag_close;
	$output = $this->full_tag_open.$output.$this->start_tag_close.$this->full_tag_close;
